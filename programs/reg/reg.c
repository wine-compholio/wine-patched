...
{
    WCHAR *buffer = NULL, *ptr = NULL;
    WCHAR hex_fmt[] = {'%','0','2','x',0};
    int i;
    ...
    switch (type)
    {
        ...
        case REG_BINARY:
            ...
            for (i = 0; i < size_bytes; i++)
                ptr += sprintfW(ptr, hex_fmt, src[i]);
            break;
        case REG_DWORD:
     /* case REG_DWORD_LITTLE_ENDIAN: */
...
static WCHAR *build_subkey_path(WCHAR *path, WCHAR *subkey_name, DWORD len)
{
    WCHAR *subkey_path;
    WCHAR fmt[] = {'%','s','\\','%','s',0};

    subkey_path = HeapAlloc(GetProcessHeap(), 0, (len + 1) * sizeof(WCHAR));
    if (!subkey_path)
    {
...
static int query_all(HKEY key, WCHAR *path, DWORD path_len, BOOL recurse)
{
    LONG rc;
    ...
    for (i = 0; i < num_subkeys; i++)
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            if (recurse)
            {
                ...
                query_all(subkey, subkey_path, subkey_path_len, recurse);
                RegCloseKey(subkey);
